  ...
  /*for(int i = 0; i < 1000; i++) 
  {
    ...
    iniVolt = iniVolt + (adc*5/1023.0)/1000;
  }*/
  ...
  const float iniVolt = x.xx; //ACS712 intial voltage w/o current flow. calibrate if needed.
  ...
  changeMUX(LOW, LOW, LOW, LOW);
  float sample = 0.0, voltage = 0.0, current = 0.0;
  for(int i = 0; i < 1000; i++) 
  {
    int adc = analogRead(analogInPin);
  ...
  output_voltage = 1.23*output_voltage; //calibrated voltage from LE-600 anemometer. calibrate for new one.
